...
public class SRoleAction : MonoBehaviour
{
...
//	}
	...
	ArrayList progressPoints = new ArrayList (); //检测动作过程的点（百分比）
	ArrayList progressCallback = new ArrayList ();	//动作过程的回调
...
//		}
		...
		if (progressCallbackInfor != null) {
			...
			for (int i=0; i<count; i++) {
				if (i % 2 == 0) {
					progressPoints.Add (Toolkit.NumEx.stringToInt (progressCallbackInfor [i].ToString ()) / 100.0f);
				} else {
	...
	int oldMotionTime = 0;		//old动作播放次数
	int MotionTime = 0;		//动作播放次数
...
//				} 
	...
	public IEnumerator exeCallback (object cbFunc)
	{
		yield return null;
		if (cbFunc != null) {
			if (typeof(LuaFunction) == cbFunc.GetType ()) {
				((LuaFunction)cbFunc).Call (this);
//				CallFunc((LuaFunction)cbFunc);
			} else if (typeof(Callback) == cbFunc.GetType ()) {
				((Callback)cbFunc) (this);
			}
		}
	}

	public object CallFunc (LuaFunction func)
	{
		int top = func.BeginPCall ();
		IntPtr L = func.GetLuaState ();
		LuaScriptMgr.Push (L, this);
		object ret = (object)LuaScriptMgr.GetLuaObject (L, -1);
		func.EndPCall (top);
		return ret;
	}
}
