...
//				spriteList.Add(tex, 2);
	...
	{
		if ((atlas == null || atlas.texture == null)  && !atlas.isBorrowSpriteMode) return null; // modify by chenbin
	...
	void UpdateAtlas_BorrowMode (List<Texture> textures)
	{
		...
		foreach (Texture tex in textures) {
			...
			foreach (DictionaryEntry  item in NGUISettings.atlas.spriteMap) {
				if (newName == (string)(item.Key)) {
					UISpriteData sp = NGUISettings.atlas.spriteList [(int)(item.Value)]; 
					sp.path = path;
			...
			s.height = tex.height;
			NGUISettings.atlas.spriteList.Add (s);
			NGUISettings.atlas.spriteMap [newName] = NGUISettings.atlas.spriteList.Count - 1;
		}
	...
	//		int startPos = 0;
	//		if(NGUISettings.atlas.useUnity3DType) {
	//			startPos = tmpPath.IndexOf ("Assets/");
	//			startPos += 7;
	//		} else {
	//			startPos = tmpPath.IndexOf ("Resources/");
	//			if (startPos < 0) {
	//				Debug.LogWarning ("该Atlas为borrow模式，资源必须放到Resources/下.error:" + tmpPath);
	//				return null;
	//			}
	//			startPos += 10;
	//		}
	//		tmpPath = tmpPath.Substring (startPos, tmpPath.LastIndexOf (".") - startPos);
	//		//Debug.Log (tmpPath);
	//		return tmpPath;
	//	}
	...
	public static string getTexturePath (Texture tex)
	{
		...
		//		tmpPath = tmpPath.Replace("/upgradeResMedium/", "/.upgradeRes/");
	...
	//		string tmpPath = AssetDatabase.GetAssetPath (tex.GetInstanceID ());
	//		if(!NGUISettings.atlas.useUnity3DType) {
	//			if (tmpPath.IndexOf ("Resources/") < 0) {
	//				Debug.LogWarning ("该Atlas为borrow模式，Resources/下.error:" + tmpPath);
	//				return null;
	//			}
	//		}
	//		if(string.IsNullOrEmpty(tmpPath)) return "";
	//		int endPos = tmpPath.LastIndexOf ("/");
	//		string tmpstr = tmpPath.Substring (0, endPos);
	//		int startPos = tmpstr.LastIndexOf ("/");
	//		tmpPath = tmpPath.Substring (startPos + 1, endPos - startPos - 1);
