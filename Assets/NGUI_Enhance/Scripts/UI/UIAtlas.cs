...
{
	//add by chenbin
	static Hashtable retainCounter = new Hashtable ();//引用计数器 
	static Hashtable assetBundleMap = new Hashtable ();//引用AssetBundle
						...
						if (objs.Length > 2) {
							...
							if (finishCallback != null) {
								if (finishCallback.GetType () == typeof(Callback)) {
									((Callback)finishCallback) (sp, spriteName, args);
								} else if (finishCallback.GetType () == typeof(LuaInterface.LuaFunction)) {
									((LuaInterface.LuaFunction)finishCallback).Call (sp, spriteName, args);
								}
							}
						}
					}
	...
	public UISpriteData borrowSpriteByname (string name, UISprite uisp, object callback, object args)
	{
		...
		if (i < 0) {
			#if UNITY_EDITOR
			Debug.LogWarning("can't find sprite ,name=[" + name + "],objname = [" + uisp.name + "]");
			#endif
			return null;
		}
...
//			rc = 1;
			...
			try {
				if (assetBundleMap [ret.path] == null) {
					...
					if(Application.isPlaying) {
                        ...
                        path = path.Replace("/upgradeRes4Publish/", "/upgradeResMedium/");
						//                            Debug.Log("get Texture==" + path);
						assetBundleMap [ret.path] = CLVerManager.self.getAtalsTexture4Edit (path);
					}
					#else
					getTuxture (ret, uisp, callback, args);
					isBorrowSprite = false;
		...
		if (callback != null) {
			if (callback.GetType () == typeof(Callback)) {
				((Callback)callback) (uisp, ret.name, args);
			} else if (callback.GetType () == typeof(LuaInterface.LuaFunction)) {
				((LuaInterface.LuaFunction)callback).Call (uisp, ret.name, args);
			}
		}
		rc++;
	...
	void getTuxture (UISpriteData ret, UISprite uisp, object callback, object args)
	{
		...
		#endif
		CLVerManager.self.getNewestRes (path, CLAssetType.assetBundle, cb, ret.path, callback, uisp, ret.name);
	}
